...
class arm_env_mj(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self,
        ...
        self.sample_shape = 6
        self.num_hist = 10
        self.past_obs = np.zeros(self.num_hist*self.sample_shape)
    ...
    def step(self, action):
        ...
        reward_y = abs(observation[3]*2)**2
        reward_z = abs(observation[1]*2)**2

        reward += -float(reward_y + reward_z)
        # reward += np.linalg.norm(self.sim.data.qpos.flat.copy()-
        t_reward = reward
        # print('Reward',reward)

        vel =  self.sim.data.qvel.flat.copy()
        reward += -(abs(vel[0]) + abs(vel[1]))/20
    ...
    def _get_obs(self):
        ...
        #################z,zg,y,yg,vy,vz,a1,a2 #############################################

        observation = np.array((position[1],(self.goalz - position[1]),position[0], \
                                (self.goaly - position[0]), \
                                velocity[0],velocity[1], \
                                self.last_action[0],self.last_action[1])).ravel()

        #Keep a history
        self.past_obs = np.append(observation,self.past_obs)
        # if self.past_obs.shape[0] > num_hist*self.sample_shape:
        self.past_obs = self.past_obs[:self.num_hist*self.sample_shape]
    ...
    def print_pos_rads(self):
        position = self.sim.data.qpos.flat.copy()
        print('Currently at:',position[0]/np.pi*180,position[1]/np.pi*180)
